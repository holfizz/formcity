...
Просто напишите свой вопрос текстом! 💬`
	...
	async handleMessage(@Message('text') message: string, @Ctx() ctx: Context) {
		...
		try {
			...
			// ШАГ 1: Проверяем, нужны ли CSV данные для ответа
			this.logger.log('Step 1: Checking if CSV data is needed...')
			const needsCsvData = await this.openAiService.checkIfNeedsCsvData(message)
			this.logger.log(`Step 1 complete: Needs CSV = ${needsCsvData}`)

			if (needsCsvData) {
				// ШАГ 2: Если нужны CSV данные - загружаем их и отправляем с запросом
				this.logger.log('Step 2: Loading CSV data...')
				const csvData = await this.csvService.getAllDataAsText()
				this.logger.log(
					`Step 2 complete: CSV data loaded, length: ${csvData.length}`
				)

				const prompt = `Сотрудник спрашивает: "${message}".

Используй данные из финансовой модели проекта для ответа. Предоставь конкретные цифры и расчеты.`

				this.logger.log(
					'Step 3: Generating response with CSV data (no web search)...'
				)
				await this.streamResponse(ctx, prompt, csvData, false) // false = БЕЗ web search
				this.logger.log('Step 3 complete: Response sent')
				this.logger.log('=== MESSAGE HANDLING COMPLETE ===')
				return
			}

			// Если CSV данные не нужны - продолжаем обычную логику
...
Проанализируй данные из базы недвижимости и предоставь:
1. Краткую сводку по найденным объектам
2. Ключевые цифры (количество, средняя цена, диапазон площадей)
3. Выводы и рекомендации

Используй только данные из контекста, не придумывай цифры.`

					await this.streamResponse(ctx, prompt, context)
